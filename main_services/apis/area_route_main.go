...
func main() {
	...
	if loadCfgErr != nil {
		utils.MsgError("[AreaAPIs]setting up area routes failed: >" + loadCfgErr.Error())
		return
	}
	...
	app := fiber.New(fiber.Config{
		// 设置请求体的最大大小（单位：字节）
		BodyLimit: 100 * 1024 * 1024 * 1024, // 50 MB
	})
	app.Use(cors.New())

	// 配置路由
	routes.SetupAreaRoutes(app, &cfg.MongoCfg, &cfg.MySqlCfg)
